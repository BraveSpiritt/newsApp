...
function App() {
    ...
    useEffect(() => {
        if (searchText === '') {
            setTitle('Please type video name');
            setVideoSrc('');
        } else {
            setTitle('');
            const debounce = setTimeout(() => {
                fetch(`https://www.googleapis.com/youtube/v3/search?part=snippet&key=AIzaSyDhHpikApA6Ecb7dKgLhgQsFLfGQUYj3Zs&maxResults=8&q=${searchText}`)
